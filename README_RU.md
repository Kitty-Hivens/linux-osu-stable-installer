...
- **Обновления самой osu!:** под Wine клиент не может записать поверх уже загруженной DLL, поэтому файлы обновления, которые он оставляет в `_pending`, ставятся на место до старта игры — иначе устаревший `osu!auth.dll` со временем закрывает клиенту доступ к серверам.
