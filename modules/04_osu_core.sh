...
create_system_integration() {
    ...
    cat > "$CONFIG_FILE" << EOF
# ==============================================================================
# osu! Linux Wrapper Configuration  (generated by osu! Installer v5.2.1)
# ==============================================================================
...
WINE_BROKEN_VERSIONS="$WINE_BROKEN_VERSIONS"
WEOF
...
# Wine releases that cannot read osu!.db. The list is baked in at install time rather than
# read back from the installer, which need not be on disk any more when the game is started.
# A launch is not blocked over it: refusing to start the game is worse than a warning, and
# the database is recoverable either way -- it is renamed, never deleted.
warn_if_wine_breaks_db() {
    local v
    v=$("$WINE_BIN" --version 2>/dev/null | grep -oE '[0-9]+\.[0-9]+' | head -n1)
    [ -n "$v" ] || return 0
    case " ${WINE_BROKEN_VERSIONS:-} " in
        *" $v "*) ;;
        *) return 0 ;;
    esac
    note -u critical "osu!" "Wine $v cannot read osu!.db. Your song list will come up empty and the real database is renamed to osu!.db.<number>.bak. Run the installer with --update to pin a working Wine."
}
