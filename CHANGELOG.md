...
### Fixed
- **`--update` and `--launch` refused to work between installing and the first launch**: both required `osu!.exe`, which by design does not exist until the client has been unpacked. `--update` now accepts the bootstrapper as evidence of an installation, and `--launch` hands over to the wrapper, which is the only thing that knows how to perform a first launch.
- **The offline guard treated an installed-but-unlaunched system as a bare machine**: re-running the installer without a connection aborted with "osu! cannot be installed offline" instead of warning and continuing. An existing prefix, a bootstrapper or an unpacked client all now count as an existing installation.
- **The bootstrap lock was held for the entire game session**: a `flock` lives on the open file description, and the descriptor was inherited by the bootstrapper and then by the client it starts. Selecting several beatmaps on a fresh installation left every wrapper but the first waiting out the full fifteen-minute timeout and importing nothing. The descriptor is now closed in the child and the lock released as soon as the client exists.
- **A beatmap could still reach a client that was mid-update**: the "wait for readiness" state was process-local, so an import started from the file manager -- a different process from the one that unpacked the client -- skipped the wait entirely. The state is now a marker in the prefix, cleared by whichever invocation first sees the client come up.
- **A dead bootstrapper was waited on for fifteen minutes**: if Wine refused the file or the bootstrapper crashed, the wait ran to the deadline before reporting anything. Its liveness is now part of the wait.
- **SteamOS was not recognised as image-based** although it is named as supported: it is not ostree-based, so neither `/run/ostree-booted` nor `rpm-ostree` is present, and its `VARIANT_ID` is `steamdeck`. Both `ID` and `VARIANT_ID` are now matched, along with `steamos-readonly`.
- **`--distrobox-name` and `--distrobox-image` were discarded unless `--distrobox` was passed too**, so `--update --distrobox-name other` silently re-entered the container recorded by the last install.
- **A custom Wine was ignored when deciding to offer container mode**: `-w /path/to/wine` on an image-based system still triggered the prompt, and declining aborted the run.
- **Container paths were validated after the image pull**, from inside the container, where a directory such as `/opt` exists in the image and passed the check while belonging to the container rather than the host -- the installation would then vanish with it. The location is now required to be under `$HOME` and is checked on the host before anything is created.
- **`--health-check` started the container** to look for `winetricks`, contradicting its promise to change nothing. A stopped container is now reported as such.
- **A warning was treated as an outcome**: when the dependency step could only warn -- an image-based host, an unknown package manager -- the run continued, `winetricks` silently did nothing inside its error-suppressing block, and the summary announced success over a prefix with no runtime at all. The essentials are verified after the step, and a genuinely missing one stops the run.
- **`--health-check` reported `winetricks` as missing on a perfectly good container**: the probe was executed directly by the container manager, which has no shell, while `command` is a shell builtin. Probes now go through `sh -c`. The check also no longer runs the Wine shim, which would have started the container a hundred lines before the guard that avoids exactly that.
- **The readiness wait was a twelve-second sleep on anything but Hyprland** -- which is every desktop the container mode targets. During a first launch the client is running but still downloading the game, so a beatmap opened then went to a client that could not take it. The first launch now waits for the bootstrapper to be gone and settles far longer, and an import the client refuses is placed in `Songs` instead of being reported as failed, where a restart or F5 picks it up.
- **A beatmap could slip through the moment between releasing the bootstrap lock and recording the first launch**: the marker is now written while the lock is still held.
- **The network precheck never ran on the host for container installs**, so an offline machine met a multi-minute image-pull failure instead of being told immediately.
- **`--distrobox-image` was silently ignored when the container already existed**; it now says so and names the command that rebuilds it.
- **`--no-distrobox`** returns to a host installation. Without it, a recorded container name made every later run rebuild the container, even after removing it by hand.
- **The `curl`-less network probe could hang for minutes** on a blackholed network, in a check whose purpose is to fail quickly. It is bounded when `timeout` is available, and still falls back to the pure shell builtin when nothing external is.
