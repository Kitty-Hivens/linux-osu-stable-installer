...
- **A beatmap the client refused was reported as failed and left where it was**: a client that is still setting itself up after a first launch cannot accept a handoff, and nothing osu! exposes says when it is done. Rather than guessing at readiness, such a beatmap is now placed in `Songs`, where a restart or F5 picks it up, and the notification says so. The file is never lost.
